...
//  FolioReaderKit
//
//  Created by Santiago Carmona on 14/01/20.
...
class RKUserResizableView: UIView {
    ...
    var _contentView: ShapeView?
    
    var contentView: ShapeView? {
        set (newValue) {
            newValue?.removeFromSuperview()
            newValue?.frame = bounds
//            newValue?.shapePath.apply((CGAffineTransform(translationX: frame.origin.x, y: frame.origin.y)))
            addSubview(newValue!)
    ...
    func setupDefaultAttributes() {
        borderView = GripViewBorderView(frame: bounds)
