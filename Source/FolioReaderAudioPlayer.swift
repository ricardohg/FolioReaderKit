...
//        try? session.setCategory(convertFromAVAudioSessionCategory(AVAudioSession.Category.playback))
    ...
    func speakSentence() {
        ...
        currentPage.webView?.js("getSentenceWithIndex('\(playbackActiveClass)')") { [unowned self] sentence in
    ...
    func updateNowPlayingInfo() {
        ...
        if let coverImage = self.book.coverImage, let artwork = UIImage(contentsOfFile: coverImage.fullHref) {
            ...
            let albumArt = MPMediaItemArtwork.init(boundsSize: artwork.size, requestHandler: { (size) -> UIImage in
                return artwork
            })
