...
open class StrokeGestureRecognizer: UIGestureRecognizer {
    // MARK: - Configuration
    open var collectsCoalescedTouches = true
    open var usesPredictedSamples = true
    
    /// A Boolean value that determines whether the gesture recognizer tracks Apple Pencil or finger touches.
    /// - Tag: isForPencil
    open var isForPencil: Bool = false {
        didSet {
            if isForPencil {
    ...
    open var outstandingUpdateIndexes = [Int: (Stroke, Int)]()
    open var coordinateSpaceView: UIView?
    
    // MARK: - State
    open var trackedTouch: UITouch?
    open var initialTimestamp: TimeInterval?
    open var collectForce = false
    
    open var fingerStartTimer: Timer?
    private let cancellationTimeInterval = TimeInterval(0.1)
    
    open var ensuredReferenceView: UIView {
