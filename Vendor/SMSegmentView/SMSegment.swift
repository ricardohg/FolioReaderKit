...
class SMSegment: UIView {
    ...
    var title: String? {
        didSet {
            ...
            if let titleText = self.label.text as NSString? {
                self.labelWidth = titleText.boundingRect(with: CGSize(width: self.frame.size.width, height: self.frame.size.height), options:NSStringDrawingOptions.usesLineFragmentOrigin , attributes: [NSAttributedString.Key.font: self.label.font as Any], context: nil).size.width
            }
            else {
                self.labelWidth = 0.0
            }
    ...
    var titleFont: UIFont = UIFont.systemFont(ofSize: 17.0) {
        didSet {
            ...
            if let titleText = self.label.text as NSString? {
                self.labelWidth = titleText.boundingRect(with: CGSize(width: self.frame.size.width + 1.0, height: self.frame.size.height), options:NSStringDrawingOptions.usesLineFragmentOrigin , attributes: [NSAttributedString.Key.font: self.label.font as Any], context: nil).size.width
            }
            else {
                self.labelWidth = 0.0
            }
